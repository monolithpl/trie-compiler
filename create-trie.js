...
Trie.prototype = {
    ...
    encode: function()
    {
        ...
        this.apply( function( node ) {
			...
			if (node.letter == ' ') value = 26;
			if (node.letter == ',') value = 27;
			if (node.letter == "'") value = 28;
			if (node.letter == "-") value = 29;
...
function go(filename) {
    var trie = new Trie();
	var words = fs.readFileSync(filename).toString().replace(/\r\n/g, '\n').split('\n')
    words.sort()
	...
	var output = [];
    output.push(trie.getNodeCount());
    output.push(directory.getData());
	output.push(trieData);
	return output;
}
function lookup(data, word) {
    try 
    {
        var ftrie = new FrozenTrie(data[2], data[1], data[0]);
         if ( ftrie.lookup(word)) console.log('yes. the word ' + word + ' was found')
			else console.log('no. the word ' + word + ' was not found');
    } catch ( e ) {
        console.log(e);
    }
}

if (process.argv[2]) var filename = process.argv[2]
else {
	console.log('please specify a wordlist file to process: node trie.js filename')
	process.exit(-1)
}
filenameOut = filename.replace(/\.[^/.]+$/, "") + '.trie'
fs.appendFileSync(filenameOut, JSON.stringify(go(filename)))

// to test the resulting trie
//lookup(JSON.parse(fs.readFileSync(filename)))